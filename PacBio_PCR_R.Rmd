---
title: "PacBio_PCR_straglr_to_downstream_analysis_steps"
output:
html_document:
df_print: paged
---
...
```{r plot_hist_after_bioawk, fig.show="hold", out.width="30%",fig.height=8, echo=T}
...
Go to folder where bam files are located and use script for extract reads and sequences from bam files and made into .txt files
```
for i in *.bam
do
ff=`echo $i | sed 's/.bam//'`
...
```{r histograms read sizes unfiltered, fig.show="hold", out.width="30%"}
...
```{r,echo=F}
tsvf = read.delim2('output.tsv',skip=1)
head(tsvf)
```
...
```{R loop the files,fig.show="hold", out.width="30%"}
